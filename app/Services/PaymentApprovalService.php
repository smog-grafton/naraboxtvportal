...
class PaymentApprovalService
{
    ...
    {
        DB::transaction(function () use (&$transaction, $skipTransactionUpdate): void {
            $transaction = PaymentTransaction::with(['user', 'transactionable', 'subscriptionPlan', 'paymentGateway'])
                ->lockForUpdate()
            ...
            $transaction->update(array_filter([
                'status' => $skipTransactionUpdate ? null : 'SUCCESS',
                'access_granted_at' => now(),
            ], fn ($value) => $value !== null));

            app(PartnerBenefitService::class)->markPaymentBenefitRedeemed($transaction);

            app(RevenueAllocationService::class)->allocateFromTransaction($transaction);
