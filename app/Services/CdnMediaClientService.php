...
class CdnMediaClientService
{
    ...
    {
        /** @var Response $response */
        $response = $this->client()->get('/api/v1/media/' . $assetId . '/playback');
    ...
    private function client(): PendingRequest
    {
        ...
        $retryTimes = max(0, (int) config('services.cdn.retry_times', 2));
        $retrySleepMs = max(0, (int) config('services.cdn.retry_sleep_ms', 800));
        $forceIpResolve = strtolower((string) config('services.cdn.force_ip_resolve', 'v4'));

        $request = Http::baseUrl($baseUrl)
            ->acceptJson()
            ->connectTimeout(max(1, (int) config('services.cdn.connect_timeout', 15)))
            ->timeout(max(1, (int) config('services.cdn.timeout', 300)));
